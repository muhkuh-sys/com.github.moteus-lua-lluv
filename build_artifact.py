...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.1-lua-lluv-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.1-lua-lluv-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.1-lua-lluv-*.xml".')
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-lua-lluv-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.4-lua-lluv-*.xml".')
